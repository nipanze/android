...
**Implemented so far (July 2026):** The schema, agreement review, and request-owner contact-unlock flow are in the app. Apply the updated `sql/cloud_patch_stage4_final.sql` in Supabase Cloud before testing Stage 4; it includes the guarded participant bid-book hardening.
...
- 🎨 **UI remaining:** aggregate-only state in `LoanDetailPage` · `TrustBadgeRow` widget · `AdvancedTrustPanel` (Pro) · post-contract review prompt
