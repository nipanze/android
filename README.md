...
| `v_lender_rate_history` | Lender interest-rate trend data for sparkline display — rate values per offer, keyed by `lender_id`. Identity-preserving: no borrower info or PII exposed. |
...
**Implemented so far (July 2026):** The schema, agreement review, contact-unlock flow, and selective transparency UI are in the app. Apply the updated `sql/cloud_patch_stage4_final.sql` in Supabase Cloud before testing Stage 4.
...
- ✅ **DB:** Lender rate history — `v_lender_rate_history` view for sparkline trend data
...
- ✅ **UI:** Selective transparency — `LoanDetailPage` renders aggregate-only view for non-participants, full detail (interest TickerCards, late fee, total payable) for owner/bidders
- ✅ **UI:** Lender rate history sparkline — lazy fetch on card expand (Pro-owner only), 6-point trend with 2-point fallback
- 🎨 **UI remaining:** `TrustBadgeRow` widget · `AdvancedTrustPanel` (Pro) · post-contract review prompt
