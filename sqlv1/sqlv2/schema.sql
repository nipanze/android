...
--
-- IMPORTANT: seed.sql inserts into auth.users FIRST with fixed UUIDs.
-- The handle_new_auth_user trigger (defined here) then syncs to
-- public.users automatically. seed.sql then UPDATEs public.users
-- to set role, status, and other fields the trigger doesn't set.
--
-- Tables (19):
--   users, user_profiles,
...
-- ============================================
-- AUTH BRIDGE FUNCTION + TRIGGER
-- Syncs auth.users → public.users on registration.
-- Placed BEFORE the users table definition so the
-- trigger exists when seed.sql inserts auth rows.
--
-- Sets only the fields auth knows about:
--   user_id = auth.users.id  (SAME UUID)
--   email   = auth.users.email
-- Everything else (role, status, etc.) gets its
-- default value and is updated by seed.sql after.
-- ============================================
...
BEGIN
    ...
    VALUES (
        NEW.id,
        NEW.email,
        '',                        -- auth handles the real hash
        'borrower',                -- default; seed.sql UPDATEs to correct role
        'pending_verification',    -- default; seed.sql UPDATEs to correct status
        FALSE,
        50,
        'bronze'
    )
...
-- This trigger fires when seed.sql (or real registration) inserts into auth.users.
-- It creates the public.users row automatically with the SAME UUID.
...
'Syncs auth.users → public.users on every new registration.
 Uses the same UUID so all FK references stay consistent.
 Fires from both real sign-ups and seed.sql auth inserts.';
...
'Weighted reputation score (0-100). Call after contract events then UPDATE users SET reputation_score = sp_calculate_reputation_score(user_id).';
...
'Credit score 300-850. Band only exposed publicly. Called by scoring-engine.';
...
-- ============================================
-- MOCK RPCs (Stage 2 — remove in Stage 4)
-- ============================================
...
BEGIN
    ...
    -- Accept chosen bid (trg_fn_lock_funds_on_accept fires here)
    UPDATE bids
    ...
    v_monthly_pmt := ROUND(
        v_bid.bid_amount
        * (v_rate_mo * POWER(1 + v_rate_mo, v_duration))
        / (POWER(1 + v_rate_mo, v_duration) - 1),
        2
    );
    ...
    INSERT INTO loan_contracts (
        request_id, borrower_id, total_amount,
        weighted_interest_rate, duration_months,
        monthly_payment, total_repayment, total_interest,
        outstanding_balance, status
    )
    VALUES (
        p_request_id, p_borrower_id, v_bid.bid_amount,
        v_bid.interest_rate, v_duration,
    ...
    INSERT INTO contract_bids (
        contract_id, bid_id, lender_id, amount, interest_rate
    )
    VALUES (
        v_contract_id, p_bid_id, v_bid.lender_id,
        v_bid.bid_amount, v_bid.interest_rate
    );
    ...
    INSERT INTO audit_logs (
        ...
        p_borrower_id, 'bid_accepted', 'contract',
        'loan_contract', v_contract_id, 'accept_bid',
...
'Atomically accepts a bid, rejects others, creates contract + contract_bids.
 Returns contract_id. Replace with Edge Function in Stage 4.';
