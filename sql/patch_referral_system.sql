-- ==============================================================================
-- NIPANZE REFERRAL & MARKETER SYSTEM — STANDALONE PATCH (v4.2)
-- ==============================================================================
-- Paste this file into your Supabase SQL Editor and run it.
-- It is fully idempotent: safe to run more than once with no side effects.
-- This is extracted from patch.sql so you don't have to re-run the whole file.
-- ==============================================================================
...
-- 2. ALTER EXISTING TABLES (IF NOT ALREADY DONE)
--------------------------------------------------------------------------------
...
CREATE POLICY "referral_rewards: own or admin read" ON public.referral_rewards
    ...
    USING (
        ...
        OR marketer_id IN (SELECT id FROM public.referral_marketers WHERE profile_id = auth.uid())
        OR private.is_admin()
    );
...
-- Internal helper: generates a unique referral code from the user's first name
...
-- Records that user was referred by someone who shared a referral code
...
BEGIN
    ...
    VALUES (
        ...
        jsonb_build_object(
            'notification_kind', 'referral_registered',
            'referral_id',       v_referral_id,
            'referred_user_id',  v_user.id
        )
    );
...
-- Returns the full referral dashboard for the current user
