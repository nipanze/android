              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                BlocBuilder<PositionsCubit, PositionsState>(
                  builder: (context, state) {
                    ...
                    final activeBids =
                        state.bids.where((b) => b.isPending).length;
                    final activeContracts = state.contracts
                        .where((c) => c['status'] == 'in_execution')
                        .length;
                    return Text(
                      [
                        if (activeBids > 0)
                          '$activeBids active bid${activeBids != 1 ? 's' : ''}',
                        if (activeContracts > 0)
                          '$activeContracts contract${activeContracts != 1 ? 's' : ''}',
                      ]
                          .join(' · ')
                          .ifEmpty('Your listings, bids & contracts'),
                      style: Theme.of(context).textTheme.bodySmall,
                    );
                  },
                ),
            ...
            child: BlocConsumer<PositionsCubit, PositionsState>(
              ...
              builder: (context, state) {
                return TabBarView(
                  ...
                  children: [
                    // ── Borrower tab ─────────────────────────────────
                    // MyListingsPage content only — no Scaffold/SafeArea
...
class _LenderTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final historical = bids
        .where((b) =>
    b.isWithdrawn ||
        b.bidStatus == BidStatus.rejected ||
...
class _ContractsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final active =
    contracts.where((c) => c['status'] == 'in_execution').toList();
    final other =
    contracts.where((c) => c['status'] != 'in_execution').toList();
